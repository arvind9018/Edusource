...
const Navbar = () => {
  ...
  const [tutorials, setTutorials] = useState([]); // State to store tutorials from Firestore
    ...
    const fetchCourses = async () => {
      try {
        ...
        const coursesList = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
          type: "course" // Add type for differentiation
        }));
  ...
  // Effect to fetch tutorials from Firestore
  useEffect(() => {
    const fetchTutorials = async () => {
      try {
        const tutorialsCollectionRef = collection(db, "tutorials");
        const querySnapshot = await getDocs(tutorialsCollectionRef);
        const tutorialsList = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
          type: "tutorial" // Add type for differentiation
        }));
  ...
  const performSearch = useCallback(
    debounce((query) => {
      if (query.length >= 3) {
        ...
        const combinedResults = [...filteredCourses, ...filteredTutorials];
              ...
              <div className="absolute top-full left-0 mt-2 w-64 bg-white text-black rounded-lg shadow-lg z-50">
                {searchResults.length > 0 ? (
                  searchResults.map(item => (
                    <Link
                      key={item.id}
                      to={item.type === "course" ? `/courses/${item.id}` : `/tutorial/${item.id}`}
